...
class HashtaggeneratorValidationException implements Exception {
  final List<String> errors;

  HashtaggeneratorValidationException(this.errors);

  @override
  String toString() => 'HashtaggeneratorValidationException: ${errors.join("; ")}';
}
...
/// Hashtag Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/hashtaggenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [text] (required) - The text to generate hashtags from
/// * [count] - Number of hashtags to generate. 1 to 10)
class HashtaggeneratorClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    ...
    'count': ValidationRule(type: 'number', required: false),
  };

  HashtaggeneratorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/hashtaggenerator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [HashtaggeneratorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw HashtaggeneratorValidationException(errors);
    }
  }

  /// Execute a request to the Hashtag Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<HashtaggeneratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw HashtaggeneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return HashtaggeneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw HashtaggeneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw HashtaggeneratorException('Resource not found');
      } else {
        throw HashtaggeneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is HashtaggeneratorException) rethrow;
      throw HashtaggeneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Hashtag Generator API client.
class HashtaggeneratorException implements Exception {
  final String message;

  HashtaggeneratorException(this.message);

  @override
  String toString() => 'HashtaggeneratorException: $message';
}
